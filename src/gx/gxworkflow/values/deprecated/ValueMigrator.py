...
class ValueMigrator:
    ...
    def migrate_runtime_to_workflowinput(self) -> None:
        ...
        for uuid, value in register.linked:
            if self.should_migrate_runtime_to_workflowinput(value):
                component = self.tool.get_input(uuid)
                workflow_input = self.create_workflow_input(component)
                self.workflow.add_input(workflow_input)
                input_value = value_utils.create_workflow_input(component, workflow_input) # bad
    ...
    def create_workflow_input(self, component: CommandComponent) -> WorkflowInput:
