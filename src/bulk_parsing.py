...
class Runner:
    ...
    def worker(self, job: JobDetails) -> None:
        """
    ...
    def get_job_details(self, xmldirs: list[str]) -> list[JobDetails]:
        """set up jobs to run"""
        details: list[JobDetails] = []
        for xmldir in xmldirs:
            xmlfiles = get_folder_xmls(xmldir)
            for xmlfile in xmlfiles:
                details.append(JobDetails(self.outdir, xmldir, xmlfile))
        return details

    def run_jobs(self, details: list[JobDetails], threads: int) -> None:
...
def main(argv: list[str]):
    tools_folder = argv[0]
    out_dir = argv[1]
    runner = Runner(tools_folder, out_dir)
    runner.run()
