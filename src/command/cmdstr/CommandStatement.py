...
class CommandStatement:
    ...
    each individual command line statement is represented as a CommandStatement.
    ...
    def get_execution_paths(self) -> Iterable[ExecutionPath]:
        ...
        default_tokens = [rtvs.get_default_token() for rtvs in self.tokens]
        for i in range(len(self.tokens)):
            tokens_copy = deepcopy(default_tokens)
            if len(self.tokens[i].tlists) > 1:
                ...
                for tlist in self.tokens[i].tlists[1:]:
    ...
    def get_galaxy_reference_count(self) -> int:
        default_tokens = [rtvs.get_default_token() for rtvs in self.tokens]
        return sum([token.gxvar is not None for token in default_tokens])
    
    def get_first_word(self) -> str:
        return self.tokens[0].tlists[0][0].text # pure cancer but 
